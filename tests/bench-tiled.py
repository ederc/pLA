...
Tiled Gaussian Elimination tester. Implemented for Intel TBB and\n\
StarPU at the moment.',
...
if int(args.inc) == -1:
  ...
  # tbb
  test_final = test_str_tbb+' -l'+args.rowsa+' -m'+args.colsa
  test_final = test_final + ' -b'+str(args.blocksize)
  for j in threads:
  ...
  # starpu
  test_final = test_str_starpu+' -l'+args.rowsa+' -m'+args.colsa
  test_final = test_final + ' -b'+str(args.blocksize)
  for j in threads:
...
# works only for GEP
  ...
  if int(max_threads) == 1:
    for k in range(0,int(args.count)+1):
      test_final = test_str_seq+' -l'+str(rowSizes[k])+' -m'+\
      str(colSizes[k])
      test_final = test_final + ' -b'+str(args.blocksize)
      print(test_final+' >> '+bench_file)
      os.system(test_final+' >> '+bench_file)
  ...
  # tbb
  test_final = test_str_tbb+' -l'+str(rowSizes[k])+' -m'+\
      str(colSizes[k])
  test_final = test_final + ' -b'+str(args.blocksize)
  for j in threads:
  ...
  # starpu
  test_final = test_str_starpu+' -l'+str(rowSizes[k])+' -m'+\
      str(colSizes[k])
  test_final = test_final + ' -b'+str(args.blocksize)
  for j in threads:
